...
public class CreatIndex {
	...
	public void creatIndex(String sql, String searchDir,String kind) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				String rt_text;
				String id;
				if (kind.equals("weibo")) {
					id = rs.getString("weiboid");
				...
				/**
				 * 此处是相对lucene4.*之前的版本改动比较大的地方，不能再直接new Field，而是new
				 * IntField，StringField，TextField等，其中
				 * TextField默认分词，StringField默认不分词
				 * ，因为我这里name和descr都需要分词，所以都用的TextField
				 * */
				doc.add(new StringField("weiboid", id, Field.Store.YES));
				doc.add(new TextField("rt_text", rt_text, Field.Store.YES));
				doc.add(new TextField("text", text, Field.Store.YES));
				doc.add(new StringField("create_time", create_time,
						Field.Store.YES));
